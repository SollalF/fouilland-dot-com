        ...
        <article className="prose-site w-full py-12">
          ...
          <section>
            ...
            <p>
              ...
              , who manually paints subpixels which gives a very cool effect.
              From afar, the image looks like it is in color, but from close by,
              you can see that the painting is actually made only of red, green,
              blue (and black).
            </p>
          ...
          <section>
            ...
            <p>
              The reason for the existence of this visual style in the first
              place is fairly interesting. Our eyes can only detect 3
              colors—red, green, and blue—due to our cone receptors in our
              eyeballs.
            </p>
            ...
            <p>
              ...
              discrete (for example, if there are a lot of very red and green
              dots next to each other), then the brain fills in the gap and
          ...
          <section>
            ...
            <p>
              ...
              associate with retro games, old-timey videos, and that we can see
              in the work of Alex_DiP.
            </p>
            <p>
              Of course, there are a lot of other factors that contribute to
              this effect:
            </p>
            <ul>
          ...
          <section>
            ...
            <p>
              For a lot of older games, enthusiasts will rely on shaders or
              actual CRT displays, as the games were intended to be played on
              such displays and were designed using tricks that made use of
              those &quot;limitations.&quot; As a result, many{" "}
              <a
          ...
          <section>
            ...
            <p>
              ...
              on multimedia processing: simply iterate over the values, get the
              R, G, and B components, and push those values to a new image that
              would contain separate pixels for each of the original pixels.
            </p>
            <p>
            ...
            <div className="my-8 not-prose flex flex-col items-center">
              ...
              <p className="text-center text-sm text-muted-foreground mt-2">
                Illustration: Pixel to 3×3 subpixel grid transformation
            ...
            <p>
              ...
              already, so it was a matter of accessing the raw data of a pixel
              and adding a bunch of zeros.
            </p>
            ...
            <p>
              ...
              fundamentally a problem for images, but for video it would be, and
            ...
            <p>
              ...
              &quot;batch&quot; the processing. For example, you process pixels
              8 by 8 and send the calculation to 8 different processing units in
              order to parallelize the processing.
            </p>
            <p>
              This is great but involves dealing with a lot of new issues:
              memory access is not the same, so we need to transfer data
              manually between CPU memory and GPU memory. Browser support for
              GPU workloads is disparate and complex.
            </p>
            <p>
            ...
            <p>
              ...
              platform-specific implementations and aims to be the future
              solution for browser-based graphics and GPU compute. One caveat:
              it is not compatible with everything yet and is still very much
              unstable (
              <a
                href="https://caniuse.com/webgpu"
                target="_blank"
            ...
            <p>
              ...
              the WebGPU project has some pretty good backers, so I&apos;m not
              too concerned.
            </p>
            ...
            <p>
              ...
              , a typed implementation of all that good WebGPU stuff. I like
              typed stuff since it reduces the learning curve in my opinion, but
              I think the library is too young to be used efficiently for big
              projects yet. I&apos;ll go into a bit more detail in the{" "}
              <a href="#camera-support">Camera Support section</a>.
            </p>

            <h3>Lower Resolution (Sampling)</h3>
            <p>
              CRT works by rendering pixels from left to right, top to bottom,
              one pixel at a time.
            </p>
            ...
            <p>
              ...
              massive images on the output, and the look of the CRT display is
              not very visible as pixel art was designed for those 480p
              displays.
            </p>
            <p>
            ...
            <p>
              This is achieved by using the value of the &quot;center&quot;
              pixel as the value of that entire block. There might be other ways
              to do it (averaging the color over the entire block, picking the
            ...
            <p>
              Note that because of the compression, especially at low
              resolutions, high-resolution images tend to become less legible,
              thus highlighting the role of pixel art artists that make design
              choices in order to maintain legibility at the cost of realism.
            </p>
            ...
            <div className="my-8 not-prose">
              ...
              <p className="text-center text-sm text-muted-foreground mt-4">
                Comparison: Intentional pixel art vs. downsampled realistic
                image. This demonstrates how downsampling cannot replace pixel
                artists and what detail and legibility is lost in the process.
              </p>
            </div>
            ...
            <p>
              Because of data transfer speed limitations, images were broadcast
              in halves—the even rows and the odd rows alternating.
            </p>
            ...
            <p>
              ...
              progressive, when all rows are rendered on each frame. This is
              where the &quot;p&quot; (progressive) and &quot;i&quot;
              (interlaced) comes from in YouTube video resolutions (e.g., 1080p,
              1080i)—not from &quot;pixels&quot; as most people might think!
            </p>

            <h3 id="camera-support">Camera Support</h3>
            <p>
              Now that the shader works, I had to make it compatible with
              cameras. This sadly meant that I kind of had to reduce a lot of
              the advantages of TypeGPU.
            </p>
            <p>
              TypeGPU does not expose all the functions that WebGPU does.
              Namely, <code>copyExternalImageToTexture()</code> does not exist
              in TypeGPU. The function I had been using,{" "}
              <code>texture.write(imageBitmap)</code>, worked fine for static
              images but could not work with the camera pipeline. I had to
              switch to using WebGPU&apos;s{" "}
              <a
            ...
            <p>
              This is at the moment the biggest limitation to TypeGPU I could
              see. You can get around it by accessing the underlying WebGPU
              objects, but that kind of defeats the point of TypeGPU. This is
              the main reason why I think one should be very careful in using
              TypeGPU for large-scale products.
            </p>
            <p>
              That being said, it does not lead to anything being impossible,
              and I simply rewrote most of the pipeline with inspiration from{" "}
              <a
            ...
            <p>
              I need to reprocess the image before re-rendering, while I can
              just add a flag to the video rendering process to export a single
              frame.
            </p>
...
await processor.startCamera(canvas);`}</code>
          ...
          <section>
            <h2>Conclusion</h2>
            <p>
              All in all, the project was very interesting and allowed me to
              dive deeper into GPU/CPU memory concepts, CRT technologies, clean
          ...
          <section>
            ...
            <h3>Bloom</h3>
            <p>
              Subpixel colors tend to bleed a bit into each other on CRT
              displays. This effectively functions as anti-aliasing and helps
              render smooth gradients when using dithering.
            </p>
            ...
            <p>
              ...
              persistence) after being activated. This leads to a bit of
              smudging which I think could help with the flickering look of
              interlaced video.
            </p>

            <h3>Inter-Pixel Space</h3>
            <p>
              Due to manufacturing limitations, older CRTs had visible black
              space between pixels that were smaller than the pixels themselves.
            ...
            <p>
              ...
              feel, despite not being CRT-specific artifacts but most commonly
              due to data transfer/camera limitations:
            </p>
            <ul>
              <li>Noise</li>
              <li>Chromatic aberration</li>
              <li>
                Lens distortion (technically to replicate the curved look of a
                CRT display)
              </li>
            </ul>
          </section>
